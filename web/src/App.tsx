...
export function DeploymentsPage({ realtime, notify }: PageProps) {
  ...
  const rollback = async (target: DeploymentTarget) => { if (!confirm(t("deployment.confirmRollback", { project: target.project_name, environment: target.environment }))) return; setBusy(`rollback:${target.id}`); try { const response = await post<{ deployment: Deployment }>(`/deployment-targets/${target.id}/rollback`, {}); targets.reload(); deployments.reload(); setDetailID(response.deployment.id); notify(t("deployment.rollbackQueued")); } catch (err) { notify(message(err)); } finally { setBusy(""); } };
  const manageContainer = async (target: DeploymentTarget, action: DeploymentContainerAction) => {
    const confirmation = action === "stop" ? "deployment.confirmContainerStop" : action === "restart" ? "deployment.confirmContainerRestart" : action === "remove" ? "deployment.confirmContainerRemove" : "";
    if (confirmation && !confirm(t(confirmation, { project: target.project_name, environment: target.environment }))) return;
  ...
  return <div className="page-stack deployment-page"><Section title={t("deployment.targets")} icon={<Rocket size={18} />} action={<button className="primary-button" onClick={openCreate}><Plus size={17} />{t("deployment.newTarget")}</button>}><DataTable headers={[t("column.project"), t("column.environment"), t("column.server"), t("deployment.containerState"), t("column.gitRef"), t("column.compose"), t("common.actions")]} empty={t("deployment.noTargets")}>{(targets.data ?? []).map(target => {
    ...
    const locked = busy !== "" || containerStatus === "pending" || !serverOnline;
    const actions: ContextMenuAction[] = [
      { id: "container-start", label: t("deployment.containerStart"), icon: Play, disabled: locked || containerStatus === "running", onSelect: () => manageContainer(target, "start") },
      { id: "container-stop", label: t("deployment.containerStop"), icon: Square, disabled: locked || containerStatus === "stopped" || containerStatus === "removed", onSelect: () => manageContainer(target, "stop") },
      { id: "container-restart", label: t("deployment.containerRestart"), icon: RefreshCw, disabled: locked || containerStatus === "stopped" || containerStatus === "removed", onSelect: () => manageContainer(target, "restart") },
      { id: "container-remove", label: t("deployment.containerRemove"), icon: Trash2, danger: true, disabled: locked || containerStatus === "removed", onSelect: () => manageContainer(target, "remove") },
      { id: "rollback", label: t("deployment.rollback"), icon: Undo2, separatorBefore: true, disabled: locked, onSelect: () => rollback(target) },
    ...
    return <tr key={target.id}><td><div className="cell-main"><strong>{target.project_name}</strong><small title={sourcePath}>{sourcePath}</small></div></td><td><Status value={target.environment} /></td><td>{target.server_name}</td><td><div className="cell-main container-state-cell"><Status value={containerStatus} />{target.container_message && <small title={target.container_message}>{target.container_message}</small>}</div></td><td><code>{target.git_ref}</code></td><td><code>{target.compose_file}</code></td><td><div className="row-actions deployment-target-actions"><ContextMenu label={t("deployment.actions")} actions={actions}><button className="primary-button small" disabled={locked} onClick={() => void run(target)}><Rocket className={busy === `deploy:${target.id}` ? "spin" : ""} size={14} />{t("deployment.deploy")}</button></ContextMenu></div></td></tr>;
