...
function DeploymentsPage({ realtime, notify }: PageProps) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      await post("/deployment-targets", { ...target, health_checks: health ? [{ type: health.startsWith("http") ? "http" : "tcp", address: health, timeout_seconds: 60 }] : [] });
      setDialog(false);
      targets.reload();
      notify(t("deployment.targetCreated"));
