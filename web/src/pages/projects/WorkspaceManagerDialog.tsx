...
export function WorkspaceManagerDialog({ open, workspace, servers, plan, planLoading, busy, error, labels, Dialog, onClose, onRename, onMove, onCopy, onLoadDeletionPlan, onDelete }: {
  ...
  return <Dialog open={open} title={workspace ? `${labels.title}: ${workspace.display_name || workspace.project_name}` : labels.title} onClose={() => { if (!busy) onClose(); }} wide className="workspace-manager-dialog-shell">
    <form className="workspace-manager" onSubmit={submit}>
      ...
      <div className="workspace-manager-content" role="tabpanel" tabIndex={0}>
